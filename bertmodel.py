...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    B, nh, T, hs = key.shape # batch size, number of heads, seq_len, dimension of each head
    S = (query @ key.transpose(-2, -1)) * (1.0 / math.sqrt(hs)) 
    S += attention_mask # note, attention mask is tensor of same shape as S with values -1e10 for pad tokens, not same setup as a4
    att = self.dropout(F.softmax(S, dim=-1)) # apply dropout after softmax normalization

    att = att @ value # (B, nh, T, T) x (B, nh, T, hs) -> (B, nh, T, hs)
    att = att.transpose(1, 2).contiguous().view(B, T, hs*nh) # maybe drop contigous
...
class BertModel(nn.Module):
  ...
  def __init__(self, config, state_dict):
    ...
    use_lora = config.fine_tune_mode == 'lora'
    print(f'use_lora, {use_lora}')
    assert not (use_lora and config.lora_pretrained), "Cannot do lora on model that was pretrained with lora"

    if config.lora_post:
      lora_layers = [name for name, m in self.named_modules() if isinstance(m, nn.Linear)]
      replace_with_lora_layers(self, lora_layers, rank=config.lora_rank)

    self.load_state_dict(state_dict)
    
    if use_lora and not config.lora_post:
  ...
  def encode(self, hidden_states, attention_mask):
    ...
    extended_attention_mask: torch.Tensor = get_extended_attention_mask(attention_mask, torch.float32)
