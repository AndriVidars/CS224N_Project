...
def train_multitask(args):
    ...
    # wrap optimizer in PCGrad
    optimizer = PCGrad(AdamW([
    ...
    for epoch in range(args.epochs):
        ...
        for i, para_batch, in tqdm(enumerate(para_train_dataloader), desc=f'train-{epoch}', disable=TQDM_DISABLE):
            ...
            try: 
                ...
                sst_loss = torch.tensor(0.0)
                sts_loss = torch.tensor(0.0)
                ...
                # gradient surgery
                if sst_loss.item() != 0.0 and sts_loss.item() != 0.0:
                    losses = [para_loss, sst_loss, sts_loss]
                elif sst_loss.item() != 0.0:
                    losses = [para_loss, sst_loss]
                elif sts_loss.item() != 0.0:
                    losses = [para_loss, sts_loss]
                else:
                    losses = [para_loss]
                optimizer.pc_backward(losses)
                optimizer.step()
                optimizer.zero_grad()
                train_loss = para_loss.item() + sst_loss.item() + sts_loss.item()
                num_batches += 1
